...
import type { ComponentDto, ConnectionDto, DiagramDto, DiagramResponse } from '@generated/model';
import { Check, ChevronDown, Clock, Code, Copy, Map, MapPinOffIcon as MapOff, Target, X } from 'lucide-react';
...
function isComponentDto(component: unknown): component is ComponentDto {
  ...
  return (
    ...
    (c.methods === undefined || Array.isArray(c.methods))
  );
}
...
function isConnectionDto(connection: unknown): connection is ConnectionDto {
  ...
  return (
    typeof c.connectionId === 'string' && typeof c.sourceMethodId === 'string' && typeof c.targetMethodId === 'string'
  );
}
...
export default function DiagramContainer({
  ...
  onVersionSelect,
  endpoint,
}: DiagramContainerProps): React.ReactElement {
  ...
  useEffect(() => {
    if (diagramData && diagramData.metadata) {
      ...
      setCurrentVersionInfo({
        versionId: isString(metadata.version)
          ? metadata.version
    ...
    (nodeId: string) => {
      setTargetNodes((prev) => {
        ...
        if (targetNode.type === 'method' && targetNode.parentId) {
          ...
          const hasSiblings = newTargets.some(
            (target) => target.parentId === targetNode.parentId && target.id !== nodeId
          );
      ...
      components.forEach((component, componentIndex) => {
        ...
        const backgroundColor =
          component.type === 'CLASS'
            ? backgroundColors[componentIndex % backgroundColors.length]
        ...
        totalHeight =
          totalHeight > CLASS_PADDING_TOP
            ? totalHeight - METHOD_VERTICAL_SPACING + CLASS_PADDING_BOTTOM
            : CLASS_PADDING_TOP + CLASS_PADDING_BOTTOM;
  ...
  useEffect(() => {
    ...
    try {
      ...
      // 노드 레이아웃 계산 (초기 위치 전달)
      const { newNodes, methodIdToNodeId, updatedPositions } = calculateLayout(
        uniqueComponents,
        expandedNodes,
      ...
      console.error(formatToKST(new Date().toISOString()), '다이어그램 데이터 처리 중 오류:', error);
    }
  ...
  return (
    ...
    >
      ...
      >
            ...
            <div className=' p-2 bg-white rounded-lg'>
              <div className='flex items-center gap-2'>
                <div className='flex flex-col'>
                  <div className='flex items-center gap-2'>
                    <code className='text-md bg-blue-50 px-2 py-1 font-mono text-gray-700 rounded'>{endpoint}</code>
                  </div>
                </div>
              </div>
            </div>
        ...
        {nodes.map((node) => (
          <NodeToolbar
            ...
            offset={10}
            className='flex items-center gap-2 p-2 bg-gray-700 rounded-md shadow-md'
          >
            {isNodeTargeted(node.id) ? (
              <button
                onClick={() => removeTargetNode(node.id)}
                className='hover:bg-red-200 flex items-center gap-1 px-3 py-1 text-gray-700 transition-colors bg-gray-100 rounded-md'
              >
                <X size={14} />
                <span>해제</span>
              </button>
            ...
            {node.type === 'method' && (
              <button
                ...
                className='hover:bg-blue-200 flex items-center gap-1 px-3 py-1 text-gray-700 transition-colors bg-gray-100 rounded-md'
              >
                <Code size={14} />
                <span>코드 상세</span>
              </button>
            ...
            <div className='flex flex-wrap gap-2'>
              {targetNodes.length > 0 ? (
                targetNodes.map((target) => (
                  ...
                  >
                    <div className='max-w-[130px] flex items-center gap-1 px-2 py-1 overflow-hidden text-xs text-red-800 bg-red-100 rounded-full'>
                      ...
                      <button
                        ...
                        className='p-0.5 hover:bg-red-300 flex-shrink-0 ml-1 transition-colors bg-red-200 rounded-full'
                    ...
                    {target.name.length > 15 && (
                      <div className='bottom-full max-w-[300px] group-hover:block absolute left-0 z-10 hidden p-2 mb-1 text-xs break-words bg-white rounded shadow-md'>
        ...
        >
          ...
          >
            <button
              ...
              className='bg-blue-50 hover:bg-gray-50 flex items-center gap-2 px-4 py-2 transition-colors rounded-md shadow-sm'
            >
              <div className='flex flex-col items-start'>
                <span className='text-md font-bold'>V {currentVersion}</span>
                {currentVersionInfo && (
                  <span className='text-ellipsis whitespace-nowrap inline-block w-24 overflow-hidden text-xs text-left text-gray-500'>
              ...
              <ChevronDown
                size={16}
                className={`text-gray-600 transition-transform ${showVersions ? 'rotate-180' : ''}`}
              />
            </button>

            {/* 버전 드롭다운 - 수직 스크롤만 허용 */}
            {showVersions && availableVersions.length > 0 && (
              <div className='w-44 max-h-60 absolute right-0 z-50 mt-1 overflow-x-hidden overflow-y-auto bg-white rounded-md shadow-lg'>
                <div className='py-1'>
                  {availableVersions.map((version) => (
                    <div
                      key={`version-${version.versionId}`}
                      className={`px-4 py-2 hover:bg-gray-100 cursor-pointer ${
                        version.versionId === currentVersion ? 'bg-blue-50' : ''
                      }`}
                        ...
                        <div className='w-32 pr-1'>
                          <span
                            className={`text-sm block text-left ${
                              version.versionId === currentVersion ? 'font-medium text-blue-700' : 'text-gray-700'
                            }`}
                          >
                            버전 {version.versionId}
                          </span>
                          <span
                            className='text-ellipsis whitespace-nowrap block w-full overflow-hidden text-xs text-left text-gray-500'
        ...
        <Panel
          ...
          className='mb-72 mr-2'
        >
          <button
            onClick={toggleMiniMap}
            className='hover:bg-gray-100 p-2 transition-colors bg-white rounded-full shadow-md'
      ...
      {codeModalOpen && (
        ...
        >
          <div
            className='max-h-[80vh] flex flex-col w-4/5 max-w-4xl bg-white rounded-lg shadow-xl'
          ...
          >
            ...
            <div className='flex justify-end p-4 border-t'>
              <button
                ...
                className='hover:bg-blue-200 flex items-center gap-2 px-4 py-2 text-blue-700 transition-colors bg-blue-100 rounded-md'
              >
                {codeCopied ? (
                  <>
                    <Check size={16} />

...
export const InterfaceNode = memo(({ data }: NodeProps<InterfaceNodeData>) => {
  ...
  return (
    <div
      className={`rounded-lg shadow-lg overflow-hidden transition-all duration-200 border-dashed ${isTargeted ? "border-red-500 animate-pulse shadow-red-100" : "border-blue-300"}`}
      style={{
    ...
    >
      ...
      <div className={`bg-gradient-to-r ${isTargeted ? "from-red-600 to-red-800" : "from-blue-600 to-indigo-700"} text-white p-3 flex items-center gap-2`}>
      ...
      {/* 설명 섹션 */}
      {description && (
        <div className={`px-4 py-3 text-xs ${isTargeted ? "text-red-700 bg-red-50 border-red-100" : "text-blue-700 bg-blue-50 border-blue-100"} border-b`}>

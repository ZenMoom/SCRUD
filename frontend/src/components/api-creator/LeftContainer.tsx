...
export default function LeftContainer({ completed, activeItem, onItemClick }: LeftContainerProps) {
  const items = [
    ...
    <div className="w-full h-full bg-white p-6 overflow-y-auto">
      ...
      <ul className="list-none p-0 m-0">
        {items.map((item) => (
          ...
          >
            {completed[item.id] ? <CheckCircle className="text-green-500 mr-2.5" size={20} /> : <XCircle className="text-red-500 mr-2.5" size={20} />}

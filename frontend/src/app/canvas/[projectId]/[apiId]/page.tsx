...
export default function CanvasPage() {
  ...
  return (
    <div className="bg-blue-50 relative p-2">
      {/* 슬라이드 패널을 위한 트리거 영역 - 작은 UI 표시 추가 */}
      <div className="absolute top-0 bottom-0 left-0 z-10 w-6 transition-all cursor-pointer group" onMouseEnter={handleMouseEnter}>
        {/* 작은 탭 표시 추가 */}
        <div className="absolute top-1/2 left-0 transform -translate-y-1/2 bg-white shadow-md rounded-r-md p-1 opacity-70 group-hover:opacity-100 transition-opacity">
          <ChevronRight size={12} className="text-gray-500 group-hover:text-blue-500 transition-colors h-16" />
        </div>
        {/* 호버 시 배경색 변경 */}
        <div className="absolute inset-0 bg-gray-200 opacity-0 group-hover:opacity-30 transition-opacity"></div>
      </div>
        ...
        <div className="flex flex-col h-full p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <button onClick={handleBackClick} className="hover:bg-gray-200 flex items-center gap-1 px-3 py-1 text-gray-700 transition-colors bg-gray-100 rounded-md">
                <ArrowLeft size={16} />
                <span>BACK</span>
              </button>
            </div>
            <button onClick={() => setApiListVisible(false)} className="hover:bg-gray-100 p-2 rounded-full">
          ...
          {activeTab === "api" && (
            <>
                      ...
                      {apiListData.map((item, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {item.endpoint && <div className="p-1 mb-1 overflow-x-auto font-mono text-sm text-gray-700 bg-gray-100 rounded">{item.endpoint}</div>}
              ...
              <div className="pt-4">
                <button onClick={completeApi} className="hover:bg-blue-600 w-full py-2 text-white transition-colors bg-blue-500 rounded">
                  현재 API 완료 처리
                </button>
              </div>
            </>
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="md:flex-row h-[calc(100vh-1rem)] flex flex-col gap-4 overflow-hidden">
          ...
          <div className="md:w-[70%] w-full h-full min-w-0" id="diagram-container">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-md mx-4 overflow-hidden transition-all transform bg-white rounded-lg shadow-xl">
            <div className="bg-blue-50 flex items-start p-4">
              ...
              <button onClick={handleCloseModal} className="bg-blue-50 hover:text-blue-700 focus:outline-none inline-flex flex-shrink-0 ml-4 text-blue-500 rounded-md">
                <X className="w-5 h-5" />
              </button>
            </div>

...
export default function CanvasPage() {
  ...
  const fetchChatData = useCallback(async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        console.error(formatToKST(new Date().toISOString()), '채팅 데이터 가져오기 오류:', err.response?.data);
    ...
    async (versionId: string) => {
      ...
      try {
        ...
        if (response.data) {
          ...
          console.error(formatToKST(new Date().toISOString()), '응답 데이터가 없습니다.');
          setError('응답 데이터가 없습니다.');
        }
      } catch (err) {
        if (axios.isAxiosError(err)) {
          console.error(formatToKST(new Date().toISOString()), '다이어그램 데이터 가져오기 오류:', err.response?.data);
  ...
  const fetchApiList = useCallback(async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        console.error(formatToKST(new Date().toISOString()), 'API 목록 가져오기 오류:', err.response?.data);
  ...
  const executeApiCompletion = useCallback(async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        console.error(formatToKST(new Date().toISOString()), 'API 완료 처리 오류:', err.response?.data);
  ...
  return (
    <div className='bg-blue-50 relative p-2'>
      ...
      <div
        className='group absolute top-0 bottom-0 left-0 z-10 w-6 transition-all cursor-pointer'
        onMouseEnter={handleMouseEnter}
      >
        {/* 작은 탭 표시 추가 */}
        <div className='top-1/2 rounded-r-md opacity-70 group-hover:opacity-100 absolute left-0 p-1 transition-opacity transform -translate-y-1/2 bg-white shadow-md'>
          <ChevronRight
            size={12}
            className='group-hover:text-blue-500 h-16 text-gray-500 transition-colors'
          />
        </div>
        {/* 호버 시 배경색 변경 */}
        <div className='group-hover:opacity-30 absolute inset-0 transition-opacity bg-gray-200 opacity-0'></div>
      </div>

...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      console.error(formatToKST(new Date().toISOString()), '프롬프트 데이터 가져오기 오류:', error.response?.data);
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (axios.isAxiosError(error)) {
      console.error(formatToKST(new Date().toISOString()), '채팅 메시지 전송 오류:', error.response?.data);
    }

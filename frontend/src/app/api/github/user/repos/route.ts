...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'contents') {
      ...
      if (!owner || !repo) {
        return NextResponse.json({ message: '저장소 소유자와 이름이 필요합니다.' }, { status: 400 });
      }
      ...
      if (!fileUrl) {
        return NextResponse.json({ message: '파일 URL이 필요합니다.' }, { status: 400 });
      }
      ...
      if (!filePath || !fileUrl) {
        return NextResponse.json({ message: '파일 경로와 URL이 필요합니다.' }, { status: 400 });
      }
    ...
    if (axios.isAxiosError(error) && error.response) {
      console.error(formatToKST(new Date().toISOString()), 'GitHub API 요청 오류:', error.response.data);

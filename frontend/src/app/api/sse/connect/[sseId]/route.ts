...
export async function GET(request: NextRequest) {
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
              ...
              if (line.includes('[디버깅] 새 토큰 수신:')) {
                ...
                try {
                  ...
                  try {
                    ...
                    console.error(formatToKST(new Date().toISOString()), 'JSON 파싱 실패:', parseError);
                  }
                } catch (e) {
                  // SSE 데이터 파싱 오류 로그 출력
                  console.error(formatToKST(new Date().toISOString()), 'SSE 데이터 파싱 오류:', e);
                }
              } else {
                ...
                try {
                  ...
                  if (line.trim().startsWith('{') && line.trim().endsWith('}')) {
                    try {
                      ...
                      // 완료 여부 확인
                      const isCompletionMessage =
                        jsonData.status === 'COMPLETED' || (jsonData.message && jsonData.message.includes('완료'));
                      ...
                      console.error(
                        formatToKST(new Date().toISOString()),
                        'JSON 형식으로 보이지만 파싱 실패:',
                        jsonError
                      );
                  ...
                  console.error(formatToKST(new Date().toISOString()), '파싱 오류:', parseError);
                  const textPayload = JSON.stringify({ text: line });
                  controller.enqueue(`data: ${textPayload}\n\n`);
          ...
          // 에러 메시지 전송
          console.error(formatToKST(new Date().toISOString()), 'SSE 처리 오류:', error);
    ...
    console.error(formatToKST(new Date().toISOString()), 'SSE 라우트 핸들러 오류:', error);

...
const getProjects = async (): Promise<Project[]> => {
  try {
    ...
    if (!token) {
      console.error(formatToKST(new Date().toISOString()), "인증 토큰이 없습니다.")
    ...
    if (response.status === 500) {
      console.error(formatToKST(new Date().toISOString()), "서버 내부 오류가 발생했습니다. 토큰 문제일 수 있습니다.")
    ...
    if (!response.ok) {
      ...
      console.error(formatToKST(new Date().toISOString()), `API 오류 (${response.status}): ${errorText}`)
    ...
    console.error(formatToKST(new Date().toISOString()), "프로젝트 목록 조회 오류:", error)
    throw error
  }
}
...
const deleteProject = async (id: string): Promise<void> => {
  try {
    ...
    console.error(formatToKST(new Date().toISOString()), "프로젝트 삭제 오류:", error)
    throw error
  }
}
...
function HomeContent() {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      ...
      try {
        ...
        console.error(formatToKST(new Date().toISOString()), err)
      } finally {
        setLoading(false)
      }
    }
  ...
  const confirmDeleteProject = async () => {
    ...
    try {
      ...
      console.error(formatToKST(new Date().toISOString()), "프로젝트 삭제 오류:", err)
    ...
    <div className="bg-gradient-to-b from-white to-gray-50 h-[calc(100vh-60px)]">
      {isAuthenticated ? (
          ...
          <div className="flex items-center justify-between mb-10">
            ...
            {isAdmin && (
              <button
                ...
                className="hover:bg-gray-700 px-4 py-2 text-white transition-colors bg-gray-800 rounded-md"
              >
                관리자 페이지
              </button>
            )}
        ...
        <div className="h-[calc(100vh-60px)] flex flex-col items-center justify-center p-4 bg-white">
          <h1 className="md:text-5xl lg:text-6xl mb-8 text-4xl font-bold text-center">
            ...
            하고{" "}
            <span className="bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent">효율적</span>
            으로
          </h1>
          <h2 className="md:text-4xl lg:text-5xl mb-16 text-3xl font-bold text-center">코드를 만들어 보세요</h2>
          <button
            onClick={handleLogin}
            className="hover:bg-gray-700 px-4 py-3 text-2xl font-medium text-white transition-all bg-gray-800 rounded-md"
          >
            SCRUD 사용해 보기
          </button>
        </div>
      ...
      {deleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="w-[350px] z-10 p-8 bg-white rounded-lg shadow-lg">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="hover:bg-red-600 px-4 py-2 text-white transition-colors bg-red-500 rounded"
              >
                삭제
              </button>
            </div>

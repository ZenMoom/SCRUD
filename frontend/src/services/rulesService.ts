...
import { RulesResponse, OperationFieldRule } from "../types/rules"
...
export const getApplicableRulesByMethod = (rules: OperationFieldRule[], method: string): OperationFieldRule[] => {
  ...
  const commonRules = rules.filter((rule) => rule.field_name === "parameters" || rule.field_name === "responses" || rule.field_name === "requestBody")

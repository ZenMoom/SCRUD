...
COMPONENT_SYSTEM_TEMPLATE = """
    ...
    2. 코드에서 interface, abstract class, 또는 @interface로 정의된 것은 type을 "INTERFACE"로 설정
    3. 메서드의 매개변수는 배열 형태로 추출(타입 정보 제외, 순수 매개변수 이름만)
    4. 메서드의 반환 타입이 코드에 명시되어 있다면 추출
    5. 클래스나 메서드의 설명은 주석이나 메시지 내용에서 관련 설명을 찾아서 추출
    6. positionX와 positionY는 이전 컴포넌트에 값이 있으면 유지, 없으면 겹치지 않도록 positionX, positionY를 설정 (컴포넌트 간 x간격: 500, y간격: 300)

    ## 중요 사항
    - 반드시 유효한 JSON 형식으로 응답해야 합니다.
    - 필수 필드(type, name)는 반드시 포함되어야 합니다.
    - type은 반드시 "CLASS" 또는 "INTERFACE" 문자열이어야 합니다.
